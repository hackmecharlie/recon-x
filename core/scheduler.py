...
async def _scan_target(ctx: ScanContext, mt: ManagedTarget) -> TargetResult:
    ...
    try:
        ...
        if not discovery_result.get("alive", False):
            logger.warning(
                "Discovery did not detect %s as alive; continuing with -Pn fallback scan for better coverage",
                label,
            )
            fallback_scan = True
        else:
    ...
    nmap_xml = ""
    try:
        target_str = target.ip or target.hostname or ""
        extra_flags = "-Pn" if fallback_scan else ""
        ...
        nmap_raw = await _run_with_timeout(nmap_task, int(ctx.config.timeout * 0.75), f"nmap:{label}")
        if nmap_raw:
    ...
    if ctx.config.ports_only:
        ...
        # SMB
        smb_ports = {445, 139}
        if (run_all and smb_ports & port_numbers) or ctx.config.smb_only:
            from modules.smb.smb_scanner import SMBScanner
            smb = SMBScanner(
                target,
                open_ports,
                username=ctx.config.smb_username,
                password=ctx.config.smb_password,
                domain=ctx.config.smb_domain,
            )
...
async def _worker(ctx: ScanContext, worker_id: str) -> None:
    ...
    while True:
        ...
        for tr in ctx.scan_result.target_results:
            current_findings.extend(tr.findings)
        try:
            from engine.deduplicator import deduplicate_findings
            deduped_findings = deduplicate_findings(current_findings)
        except Exception:
            deduped_findings = current_findings
        ...
        cp_data = CheckpointData(
            ...
            findings=[f.to_dict() for f in deduped_findings],
