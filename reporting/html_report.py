...
def _build_template_context(
    ...
    Args:
        ...
        include_empty_targets: Whether to keep hosts with empty scans in the report.
        include_unreachable_targets: Whether to keep unreachable hosts in the report.
    ...
    # Filter targets based on report visibility settings
    visible_results = []
    hidden_results = []
    for tr in scan_result.target_results:
        is_unreachable = tr.status == "unreachable"
        is_empty = (
            tr.status == "completed"
            and not tr.open_ports
            and not tr.findings
            and not tr.cert_result
            and not tr.screenshots
            and not tr.banners
            and not tr.smb_info
        )
        if (is_unreachable and not include_unreachable_targets) or (is_empty and not include_empty_targets):
            hidden_results.append(tr)
        else:
            visible_results.append(tr)

    reachable_hosts = [tr.target.display_name for tr in scan_result.target_results if tr.status != "unreachable"]
    unreachable_hosts = [tr.target.display_name for tr in scan_result.target_results if tr.status == "unreachable"]
    ...
    for tr in visible_results:
        target_statuses.append({
            ...
            "ip": tr.target.ip or "",
            "status": tr.status,
            "reachable": tr.status != "unreachable",
    ...
    for tr in visible_results:
        if tr.smb_info:
            ...
            smb_details.append({
                ...
                "session_type": (
                    "Authenticated"
                    if getattr(info, "authenticated", False)
                    else "Null/Guest"
                ),
                "smb_version": getattr(info, "smb_version", ""),
                "access": ", ".join([s.get("name") for s in getattr(info, "shares", []) if s.get("name")]) or "No shares",
                "notes": ", ".join(
                    [
                        "Authenticated" if getattr(info, "authenticated", False) else "Unauthenticated",
                        "Guest access" if getattr(info, "guest_access", False) else "No guest access",
                    ]
                ),
            })

    smb_data = {
        "authenticated_sessions": [item for item in smb_details if item["session_type"] == "Authenticated"],
        "null_sessions": [item for item in smb_details if item["session_type"] == "Null/Guest"],
        "details": smb_details,
    }
    ...
    return {
        ...
        "unreachable_targets": scan_result.unreachable_targets,
        "hidden_targets_count": len(hidden_results),
