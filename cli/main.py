...
def _pull_latest_repo(branch: str = "main", remote: str = "origin") -> None:
    """Attempt to update the local repository from Git."""
    repo_dir = Path(__file__).resolve().parent.parent
    if not (repo_dir / ".git").exists():
        console.print("[yellow]Warning:[/] No git repository detected — cannot auto-update.")
        return

    console.print(f"[dim]Updating repository from {remote}/{branch}...[/]")
    try:
        proc = subprocess.run(
            ["git", "-C", str(repo_dir), "pull", remote, branch],
            capture_output=True,
            text=True,
            check=False,
        )
        if proc.returncode == 0:
            console.print(f"[green]Repository updated successfully.[/]")
            if proc.stdout:
                console.print(proc.stdout.strip())
        else:
            console.print(f"[red]Repo update failed:[/] {proc.stderr.strip() or proc.stdout.strip()}")
    except FileNotFoundError:
        console.print("[red]Git is not installed or not available in PATH.[/]")
...
def scan(
    ...
    auto_update: bool = typer.Option(False, "--auto-update", help="Pull latest code from the repository before running"),
    include_empty_targets: bool = typer.Option(
        False,
        "--include-empty-targets",
        "--show-all-targets",
        "--complete-scan",
        help="Include hosts with empty scan results in the generated report",
    ),
    include_unreachable_targets: bool = typer.Option(False, "--include-unreachable-targets", help="Include unreachable hosts in the generated report"),
    # SMB credential options
    smb_username: Optional[str] = typer.Option(None, "--smb-username", help="SMB username for authenticated SMB scans"),
    smb_password: Optional[str] = typer.Option(None, "--smb-password", help="SMB password for authenticated SMB scans"),
    smb_domain: Optional[str] = typer.Option(None, "--smb-domain", help="SMB domain or workgroup"),
    ...
    [dim]recon-x resume[/]
    [dim]# Include empty targets in the final report[/]
    [dim]recon-x scan -t targets.txt --include-empty-targets[/]
    [dim]# Include empty targets in the final report[/]
    [dim]recon-x scan -t targets.txt --include-empty-targets[/]
    """
    ...
    if not smb_username and not smb_password and (smb_only or not (ports_only or web_only or tls_only or screenshots_only)):
        if Prompt.ask(
            "Do you want to provide SMB credentials for authenticated SMB enumeration?",
            choices=["y", "n"],
            default="n",
        ).lower() == "y":
    ...
    _run_scan(
        ...
        tls_only=tls_only,
        screenshots_only=screenshots_only,
        include_empty_targets=include_empty_targets,
        include_unreachable_targets=include_unreachable_targets,
        smb_username=smb_username,
...
def report(
    ...
    report_html = Path(checkpoint_dir) / "report.html"
    report_pdf = Path(checkpoint_dir) / "report.pdf"
    cp_data = cp_mgr.load()
    if not cp_data:
        if report_html.exists() or report_pdf.exists():
            console.print(f"[yellow]No checkpoint found in {checkpoint_dir}. Existing report files are already present and were not overwritten.[/]")
            if report_html.exists():
                console.print(f"[green]Existing HTML report:[/] {report_html}")
            if report_pdf.exists():
                console.print(f"[green]Existing PDF report:[/] {report_pdf}")
            raise typer.Exit(0)

        console.print(f"[red]No checkpoint found in {checkpoint_dir}[/]")
        raise typer.Exit(1)

    if not cp_data.findings:
        if report_html.exists() or report_pdf.exists():
            console.print(f"[yellow]Checkpoint has no findings; existing report files were preserved.[/]")
            if report_html.exists():
                console.print(f"[green]Existing HTML report:[/] {report_html}")
            if report_pdf.exists():
                console.print(f"[green]Existing PDF report:[/] {report_pdf}")
            raise typer.Exit(0)
        console.print(f"[red]Checkpoint contains no findings and cannot regenerate a report.[/]")
        raise typer.Exit(1)
...
@app.command()
def update(
    branch: str = typer.Option("main", "--branch", "-b", help="Git branch to pull from"),
    remote: str = typer.Option("origin", "--remote", "-r", help="Git remote to pull from"),
) -> None:
    """Update the local RECON-X repository from Git."""
    _pull_latest_repo(branch=branch, remote=remote)
...
def _run_scan(
    ...
    include_empty_targets: bool = False,
    include_unreachable_targets: bool = False,
    ...
    config = ScanConfig(
        ...
        tls_only=tls_only,
        screenshots_only=screenshots_only,
        smb_username=smb_username or "",
        smb_password=smb_password or "",
        smb_domain=smb_domain or "",
    )
    ...
    # Preserve the final checkpoint file on successful completion.
    # This allows report regeneration from completed scan output later.
    # if manager.is_done():
    #     cp_mgr.delete()
...
def _generate_reports(
    ...
    Args:
        ...
        include_empty_targets: Whether to include empty-scan hosts in reports.
        include_unreachable_targets: Whether to include unreachable hosts in reports.
    """
