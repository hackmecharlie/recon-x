...
class SMBScanner:
    ...
    def __init__(self, target: Any, open_ports: List[Port], username: str = "", password: str = "", domain: str = "") -> None:
        """Initialize SMBScanner.

        Args:
            target: Target object with ip and hostname.
            open_ports: List of open Port objects from nmap.
            username: Optional SMB username for authenticated enumeration.
            password: Optional SMB password.
            domain: Optional SMB domain or workgroup.
        """
        self.target = target
        self.host = target.ip or target.hostname or ""
        self.open_ports = open_ports
        if any(p.port_number == 445 for p in open_ports):
            self.port = 445
        elif any(p.port_number == 139 for p in open_ports):
            self.port = 139
        else:
            self.port = 445
    ...
    def _enumerate_smb(self, info: SMBInfo) -> None:
        ...
        try:
            conn = SMBConnection(self.host, self.host, sess_port=self.port, timeout=_SMB_TIMEOUT)
        ...
        # ── Test authenticated login (if credentials provided) ─────────────
        auth_conn: Optional[Any] = None
        ...
        try:
            # Prefer authenticated session, then null session, then guest access
            if info.authenticated and auth_conn:
                enum_conn = auth_conn
            elif info.null_session:
                enum_conn = SMBConnection(self.host, self.host, sess_port=self.port, timeout=_SMB_TIMEOUT)
                enum_conn.login("", "", "", "", "")
