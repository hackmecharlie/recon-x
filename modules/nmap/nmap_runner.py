...
_PROFILE_FLAGS: Dict[str, str] = {
    "quick": "-T4 --top-ports 200 --max-retries 3",
    "normal": "-T3 -sV -sC --top-ports 2000 --max-retries 3",
    "full": "-T3 -sV -sC -O --top-ports 5000 --script=vuln --max-retries 3",
}

_DISCOVERY_FLAGS = "-sn -PE -PP -PS22,80,139,443,3389 -PA80,443,139 --max-retries 3 -T4"
...
class NmapRunner:
    ...
    def _host_is_up(self, xml: str) -> bool:
        """Heuristically determine whether nmap saw the host as up."""
        if not xml.strip():
            return False
        return 'status state="up"' in xml or "<host>" in xml
    ...
    def quick_discovery(self, target: str) -> Dict[str, Any]:
        ...
        xml = result.get("xml", "")
        alive = self._host_is_up(xml)
        return {"alive": alive, "xml": xml}

    def run_scan(self, target: str, extra_flags: str = "") -> Dict[str, Any]:
        """Run a full scan using the configured profile.

        Args:
            target: IP address or hostname.
            extra_flags: Additional flags to append to the profile flags.

        Returns:
            Dict with 'xml' (raw nmap XML), 'returncode', and optionally 'error'.
        """
        flags = f"{_PROFILE_FLAGS[self.profile]} {extra_flags}".strip()
